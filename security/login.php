...
if(!empty($_POST)){
    ...
    if (empty($_POST['email'])) {
        ...
        //verification de l'existence d'un utilisateur à cette adresse
        if ($user->rowCount() == 1) {

            $user=$user->fetch(PDO:: FETCH_ASSOC);
            // le mail existe on verifie le mdp
            ...
            if(password_verify($_POST['password'],$user['password'])){
                ...
                header('location:../');// remonte au dessus index.php sans l'afficher
                exit(); // => header pour traiter la session et ses messages
